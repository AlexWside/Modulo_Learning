      ...
      <div  class="container">
        <div  class="row">
          ...
          <?php
                    ...
                    foreach($Lista as $item){ // inicio geral item
                      $usuarios_permitidos = $arquivo->buscar_permissao($item[0]);
                      $bool = false; // variavel de controle
                      foreach($usuarios_permitidos as $permitido){ // foreach busca todos da lista de permitidos
                          if($permitido[1] == $_SESSION['matricula']){ // compara com cada um dos que vem da API
                            $bool = true; // muda o status da variavel de controle caso encontre 
                          }// fim if de status
                        }
                      if($item[11] == 0 && $_POST['id_assunto'] == $item[7] && $bool == true){ // if configuração de exibição
                        ?>
                    <!-- inicio de um item forech-->
          <div class="col-lg-4 d-none d-lg-block">
            <div style="border-radius:10px; height:480px; box-shadow: 10px 5px 5px black; font-size: 14px; " class="card">
                  <?php 
                  if(!$item[2]==null){
