...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar') {
  if (isset($_POST['usuarios_permitidos'])) { // if tratamento de input array vazio no adicionar
    if (!isset($_SESSION['matricula'])) {
      die('Sem sessao! Favor logar novamente.');
    }
    foreach ($_POST['usuarios_permitidos'] as $permitido) {
      if ($arquivo->buscar_usuario_permissao($_POST['id_curso'], $permitido)) {
        continue;
      } else {
    ...
    echo "selecione alguem a quem deseja adicionar";
  }
...
if (isset($_POST['remocao']) && $_POST['remocao'] == 'editar') {
  if (isset($_POST['remover_permitidos'])) {
    ...
    echo "selecione alguem a quem deseja remover";
  }
} // fim remover 
...
<body>
  ...
  <div class="container exibicao">
    ...
    <form class="container" action="" method="POST">
      ...
      <input type="submit" class="btn btn-danger botao" onclick="waittoinput()" value="Remover" />
      ...
      <select id="select-usuarios-permitidos" name="remover_permitidos[]" multiple="multiple">
        ...
        ?>
          <option value='<?= $permitido['id'] ?>' data-section='<?= $usuario->buscar_nome_usuario($permitido['matricula'])['grupo'] ?>' data-description=<?= $permitido['matricula'] ?>><?= $usuario->buscar_nome_usuario($permitido['matricula'])['nome'] ?></option>
        <?php
  ...
  <form class="container exibicao" action="" method="POST">
    ...
    <input type="hidden" name="id_curso" value=<?= $_POST['id_curso'] ?>>
    <input type="submit" class="btn btn-primary botao" onclick="waittoinput()" value="Adicionar" />
    <button type="button" style="background-color:red; margin:0.5%;" class="btn">
      <a style="color:#FFF; text-decoration:none;" href="meus_treinamentos"><i class="fas fa-arrow-circle-left"></i> Voltar</a>
    </button>
    <select id="select-usuarios" name="usuarios_permitidos[]" multiple="multiple">
      <?php foreach ($bigList as $key => $values) {  // esse foreach puxa todos os usuarios da API
        ...
        foreach ($usuarios_permitidos as $permitido) { // foreach busca todos da lista de permitidos
          if ($permitido['matricula'] == $key) { // compara com cada um dos que vem da API
